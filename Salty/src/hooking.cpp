...
namespace big
{
	...
	static bool is_owner(rage::CNetGamePlayer* src, rage::netObject* netObject) { return src != nullptr && netObject != nullptr && src->player_id != -1 && (src->player_id == netObject->syncData.ownerId || src->player_id == netObject->syncData.nextOwnerId); }
	...
	static void clone_create_ack(rage::CNetworkObjectMgr* mgr, rage::CNetGamePlayer* src, rage::CNetGamePlayer* dst, uint16_t object_id, int32_t ack_code)
	{
		...
		__try { g_hooking->m_clone_create_ack_hook.get_original<functions::clone_create_ack_t>()(mgr, src, dst, object_id, ack_code); }
	...
	static void clone_sync_ack(rage::CNetworkObjectMgr* mgr, rage::CNetGamePlayer* src, rage::CNetGamePlayer* dst, rage::datBitBuffer *buffer, uint16_t object_id, int32_t ack_code)
	{
		...
		__try { g_hooking->m_clone_sync_ack_hook.get_original<functions::clone_sync_ack_t>()(mgr, src, dst, buffer, object_id, ack_code); }
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			features::sync++;
	...
	static void clone_remove_ack(rage::CNetworkObjectMgr* mgr, rage::CNetGamePlayer* src, rage::CNetGamePlayer* dst, uint16_t object_id, int32_t ack_code)
	{
		...
		__try { g_hooking->m_clone_remove_ack_hook.get_original<functions::clone_remove_ack_t>()(mgr, src, dst, object_id, ack_code); }
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			features::sync++;
	...
	static void clone_remove(rage::CNetworkObjectMgr* mgr, rage::CNetGamePlayer* src, rage::CNetGamePlayer* dst, int32_t _object_id, int32_t unk)
	{
		...
		__try { g_hooking->m_clone_remove_hook.get_original<functions::clone_remove_t>()(mgr, src, dst, _object_id, unk); }
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			features::sync++;
	...
	static bool sync_can_apply(rage::netSyncTree* netSyncTree, rage::netObject* netObject) //this entire function needs more testing
	{
		...
		__try { return g_hooking->m_sync_can_apply_hook.get_original<functions::sync_can_apply_t>()(netSyncTree, netObject); }
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			features::sync++;
	...
	static bool script_event(int32_t group, int32_t event_id, int64_t* data, int32_t n)
	{
		...
		__try { g_hooking->m_script_event_hook.get_original<functions::script_event_t>()(group, event_id, data, n); }
		__except (EXCEPTION_EXECUTE_HANDLER)
		{
			features::script++;
	...
	void hooking::enable()
	{
		...
		for (auto native : misc::natives_replace) { m_native.emplace(native.first, native.second); }

		for (auto native : misc::natives_logging) { m_native.emplace(native.first, native.second); }

		for (auto hash : g_pointers->m_script_program_table->all_script()) { m_native_hook.push_back(new script_hook(hash, m_native)); }
