...
namespace big::misc
{
	...
	struct user
	{
		...
		user(int64_t user_id) :event(0), create(0), timestamp(0), block_time(0), script_time(0), respawn_time(0), pack_time(0), is_friend(false), is_joining(false), player_id(0)
		{
	...
	bool block_proto(rage::CNetGamePlayer* src, int32_t sync_type, int16_t object_type, int16_t object_id)
	{
		if (src != nullptr)
		{
			...
			if (users[user_id]->is_joining && sync_type == rage::CREATE && object_type != rage::PLAYER) 
				blocked = true;

			//if joining, no sync or remove
			if (users[user_id]->is_joining && (sync_type == rage::SYNC || sync_type == rage::REMOVE)) 
				blocked = true;

			//remove player id
			if (sync_type == rage::REMOVE && object_id == users[user_id]->player_id) 
				blocked = true;

			//duplicate object, ignoring the odd repeats
			if (sync_type == rage::CREATE && object_type != rage::DOOR && users[user_id]->object_id[object_id]++ >= 3) 
				blocked = true;

			//change object type
			if (sync_type == rage::SYNC && users[user_id]->object_type[object_id] != object_type) 
				blocked = true;

			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

			//remove objects
			if (sync_type == rage::REMOVE) 
				users[user_id]->object_id[object_id] = 0;
	...
	bool block_user(rage::CNetGamePlayer* src, bool blocked) //fuck you
	{
		////source is valid
		//if (src != nullptr
		//	//&& src->is_valid() 
		...
		if (src != nullptr)
		{
			...
			int64_t user_id = src->get_net_data()->m_rockstar_id;
			if (users.find(user_id) == users.end()) users[user_id] = new user(user_id);
			return now < users[user_id]->block_time;
		}
	...
	{
		static int16_t allowed[] = { GIVE_CONTROL_EVENT, REMOTE_SCRIPT_INFO_EVENT, REMOTE_SCRIPT_LEAVE_EVENT };
...
#define SIG(index, a) {index, LEN(a), a}
	...
	void* vtable(void* object, int function) { return ((void***)object)[0][function]; }
