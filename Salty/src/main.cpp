	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		g_main_thread = CreateThread(nullptr, 0, [](PVOID) -> DWORD
		{
			...
			auto logger_instance = std::make_unique<logger>();
			{
				...
				LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] INITIALIZING RENDERER";

				auto renderer_instance = std::make_unique<renderer>();

				LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] CREATING UNIQUE FIBER POOL";

				auto fiber_pool_instance = std::make_unique<fiber_pool>(10);

				LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] HOOK ATTEMPT ONE (1)";
			...
			LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] UNHOOKING AND SHUTTING DOWN";
			logger_instance.reset();

			LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] CLOSE HANDLE PROCESS";

			CloseHandle(g_main_thread);

			LOG(RAW_GREEN_TO_CONSOLE) << "[LOG] FAREWELL!";

			//LOG(RAW_GREEN_TO_CONSOLE) << (fmt::format("FREED LIBRARY: {}", g_hmodule).c_str());
